...
| Deploy | Docker Compose locally; Render (API + Postgres) + Vercel (frontend) for the demo — see D114 |
...
| "Redis pub/sub for horizontal scaling" | ✅ true **in CI; not live** | `tests/multi_instance_api_test.go` — two fully wired instances, one Postgres, one Redis | instance B's `OpTransport` replaced with `NoopTransport`; reconcile interval pushed to 5 minutes so the log-repair path cannot cover for it. **The live Render deploy runs single-instance, no `REDIS_URL` (D114)** — deliberate, and said so here rather than only in `docs/deploy.md` |
...
| D114 | Deploy retargeted from Fly.io to **Render (API + Postgres) and Vercel (frontend)**, and Render's health check points at `/livez`, **not** `/readyz` | Fly's free allowance still asks for a card at signup; Render's genuinely does not — a cost decision, not a technical one, made explicit rather than left implicit in a changed config file. The `/livez` choice IS technical and is the one part of this retarget that isn't a straight port: Render has a single health-check slot that does double duty, gating traffic during deploys **and** restarting the instance after 60s of continuous failures. Pointing it at `/readyz` — correct on Fly, where a separate, forgiving `/livez` handled restarts — would mean a Postgres blip that `/readyz` is designed to fail (D111) could, after a minute, get the whole process restarted by Render itself. That is precisely the outcome D110 exists to prevent. No deploy-time gap opens from the switch: the app fails fast on an unreachable Postgres *before* it starts listening, so if the database is down at boot, `/livez` is simply never reachable and the deploy correctly stalls. `HTTP_DRAIN_DELAY` was also retuned down (8s → 5s) rather than copied, with the difference stated honestly: Fly's number was sized against a load balancer polling readiness every 2s, which Render's single free-tier instance has no equivalent of, so the shorter number is a cheap defensive margin, not a proven-necessary one on this platform. Two things researched and found genuinely unconfirmable, written down in `docs/deploy.md` rather than asserted with false confidence: whether `preDeployCommand` (Render's `release_command` equivalent) behaves identically for a `runtime: docker` service as for buildpack ones, and whether Render's edge overwrites or appends `X-Forwarded-For` on a client-supplied header — the latter matters because `TrustProxyHeaders` enables chi's `middleware.RealIP`, which the chi maintainers themselves mark deprecated and spoofable, and turning it off trades a possible spoofing hole for a certain one (every real user sharing Render's own proxy IP in the rate limiter). Left on, on balance, and flagged for verification rather than silently resolved either way |
...
- **Stage 4 — Demo polish** 🚧: intentional UI ✅ (Stage 3 Slice 3), seed script ✅ (Slice 2,
  ...
  - ✅ Slice 3 — **operational readiness and deploy.** Decisions D110–D113. The frontend is
    ...
    - ✅ **Shutdown drains before it stops accepting** (D112). Readiness goes false, the
      ...
      listener closed after it. This mechanism, and the reasoning behind it, is
      platform-independent application design — it did not change in D114's retarget below.
    - ✅ **Dockerfile** — multi-stage, static, `scratch` plus CA certificates. Two Stage-1
      ...
      confirmed reporting `a64349a8bf60-dirty` from a real image build. Reused unchanged by
      D114 — the image was never Fly-specific.
    - ✅ **6 tests** (`tests/health_api_test.go`), each verified against the specific planted
      ...
      confirmation — see below. Platform-independent; untouched by D114.
    - ➡️ **Deploy target retargeted from Fly.io to Render — see D114.** The original
      `fly.toml` and its reasoning (probe contract mapped onto Fly's two-check model, the
      `kill_timeout` margin arithmetic) are gone from the tree, not merely superseded in
      place, because keeping a non-functional `fly.toml` beside docs that no longer describe
      it invites someone to try it and be confused. Git history has it if it's ever needed
      again. `docs/deploy.md` and `render.yaml` are the current source of truth.
